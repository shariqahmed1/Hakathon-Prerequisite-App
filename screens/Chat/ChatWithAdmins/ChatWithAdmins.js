...
export default class App extends React.Component {
  ...
  _fetchMessages = () => {
    let { userData, messages } = this.state;
    FIREBASE_DATABASE.ref('admin').child('messages').child(userData.id).on('value', snap => {
      messages = [];
      snap.forEach(snapshot => {
        messages.push(snapshot.val())
      })
      this.setState({ 
        messages: _.sortBy(messages, [function(o) { return o.timeStamp; }]), 
  ...
  _msgSend = () => {
    const { userData, msgTxt } = this.state;
    ...
    if (msgTxt) {
      ...
      FIREBASE_DATABASE.ref('admin').child('messages').child(myId).push({
